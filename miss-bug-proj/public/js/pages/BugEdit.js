...
export default {
  ...
  methods: {
    saveBug() {
      if (!this.bug.title || !this.bug.severity || !this.bug.description) eventBusService.emit('show-msg', { txt: 'All fields must be filled out.', type: 'error' })
      else
        bugService.save(JSON.parse(JSON.stringify(this.bug))).then(() => {
